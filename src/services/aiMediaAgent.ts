...
export function normalizeMediaFilename(input: string): string {
  return input
    .replace(/\.[^.]+$/, "")
    .replace(/[._-]+/g, " ")
    .replace(/\b(1080p|720p|2160p|4k|bluray|webrip|x264|x265|h264|h265|aac|dts)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim();
}
...
export function shouldQuarantineDuplicate(hashA?: string, hashB?: string): boolean {
