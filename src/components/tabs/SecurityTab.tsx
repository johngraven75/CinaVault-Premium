...
export default function SecurityTab() {
  const { vpnConnected, vpnLocation, setVpnStatus, addStatusMessage } =
    useAppStore();
  ...
  const connectVpn = async () => {
    ...
    try {
      const result = await invoke<any>("vpn_connect", {
  ...
  const disconnectVpn = async () => {
    ...
    try {
      ...
      addStatusMessage(`VPN disconnect error: ${e}`);
    }
    setVpnLoading(false);
  };
  ...
  const runScan = async () => {
    ...
    try {
      ...
      addStatusMessage(`Scan failed: ${e}`);
    }
    setAvScanning(false);
  };
  ...
  const updateSignatures = async () => {
    ...
    try {
      ...
      addStatusMessage(`Update failed: ${e}`);
    }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="glass-panel p-5">
        <h3 className="text-sm font-bold mb-4 flex items-center gap-2">
          <Scan size={16} className="text-cv-accent" /> Antivirus — Windows
          Defender
        </h3>
