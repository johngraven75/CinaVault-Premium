...
export default function LiveTVTab() {
  ...
  const removeProfile = async (id: number) => {
    try {
      ...
      loadProfiles();
      if (selectedProfile === id) { setSelectedProfile(null); setChannels([]); }
    } catch (e) { addStatusMessage(`Failed: ${e}`); }
  };
  ...
  const playChannel = async (streamUrl: string) => {
    try {
      ...
      addStatusMessage("Playing channel...");
    } catch (e) { addStatusMessage(`Playback failed: ${e}`); }
  };
            ...
            {Object.entries(groupedChannels).map(([group, chs]) => (
              <div key={group}>
                ...
                {chs.map((ch: any, i: number) => (
                  <div key={ch.id || i} className="px-5 py-2 flex items-center gap-3 zebra-row cursor-pointer" onClick={() => playChannel(ch.stream_url)}>
