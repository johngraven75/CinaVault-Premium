...
import { Tv, Plus, Trash2, RefreshCw, Play, Radio, Wifi, Globe, List, Search, Calendar, ArrowLeft } from "lucide-react";
...
export default function LiveTVTab() {
  ...
  const [newProfile, setNewProfile] = useState({ name: "", server_url: "", username: "", password: "" });
  ...
  const removeProfile = async (id: number) => {
    try {
      ...
      loadProfiles();
      if (selectedProfile === id) { setSelectedProfile(null); setChannels([]); setSelectedChannel(null); }
    } catch (e) { addStatusMessage(`Failed: ${e}`); }
  };
  ...
  const syncStreams = async (id: number) => {
    ...
    try {
      ...
      loadChannels(id);
    } catch (e) { addStatusMessage(`Sync failed: ${e}`); }
  };
  ...
  const syncEpg = async (id: number) => {
    ...
    } catch (e) { addStatusMessage(`EPG sync failed: ${e}`); }
  };
  ...
  const loadChannels = async (profileId: number) => {
    ...
    try {
      ...
      setChannels(ch);
    } catch { setChannels(DEMO_CHANNELS); }
  };

  const filteredChannels = channels.filter(ch =>
    !channelSearch || ch.name.toLowerCase().includes(channelSearch.toLowerCase())
  );

  const groupedChannels = filteredChannels.reduce((acc: Record<string, any[]>, ch) => {
      ...
      <div className="glass-panel p-5">
        ...
        {profiles.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {profiles.map(p => (
              <div key={p.id} className={`glass-panel-2 p-4 rounded-lg cursor-pointer transition-all ${selectedProfile === p.id ? "ring-1 ring-cv-accent" : "hover:bg-white/5"}`}
                    onClick={() => loadChannels(p.id)}>
                 <div className="flex items-center justify-between mb-2">
                   <span className="text-sm font-semibold">{p.name}</span>
                   <span className={`status-dot ${p.enabled ? "online" : "offline"}`} />
                 </div>
                <div className="text-[10px] text-cv-subtext mb-3 truncate">{p.server_url}</div>
                <div className="flex gap-1">
                  <button onClick={(e) => { e.stopPropagation(); syncStreams(p.id); }} className="cv-btn cv-btn-secondary text-[10px] py-1 px-2"><RefreshCw size={10} /> Sync</button>
                  <button onClick={(e) => { e.stopPropagation(); syncEpg(p.id); }} className="cv-btn cv-btn-secondary text-[10px] py-1 px-2"><Calendar size={10} /> EPG</button>
                  <button onClick={(e) => { e.stopPropagation(); removeProfile(p.id); }} className="cv-btn cv-btn-danger text-[10px] py-1 px-2"><Trash2 size={10} /></button>
                </div>
              </div>
            ))}
          </div>
           ...
           {selectedChannel ? (
             <div className="flex flex-col h-full">
               <div className="px-5 py-3 border-b border-white/5 flex items-center justify-between">
                 <button onClick={() => setSelectedChannel(null)} className="text-cv-accent hover:text-cv-accent/80 transition-colors">
                   <ArrowLeft size={16} /> Back
                 </button>
                 <h3 className="text-sm font-bold truncate max-w-xs">{selectedChannel.name}</h3>
               </div>
                 ...
                 {Object.entries(groupedChannels).map(([group, chs]) => (
                   <div key={group}>
                     ...
                     {chs.map((ch: any, i: number) => (
                       <div key={ch.id || i} className="px-5 py-2 flex items-center gap-3 zebra-row cursor-pointer" onClick={() => setSelectedChannel(ch)}>
                         <div className="w-8 h-8 rounded bg-white/5 flex items-center justify-center shrink-0">
                           {ch.logo_url ? <img src={ch.logo_url} className="w-6 h-6 object-contain" /> : <Tv size={14} className="text-cv-subtext" />}
                         </div>
...
const DEMO_PROFILES = [
  { id: 1, name: "Premium IPTV", server_url: "http://iptv.example.com:8080", username: "user", password: "pass", enabled: true, last_synced: null },
];
const DEMO_CHANNELS = [
