...
function buildPublicUrl(publicIp: string, port: string, secure: boolean): string {
...
export default function ServerTab() {
  ...
  const [customUrl, setCustomUrl] = useState(serverUrl || "http://localhost:8096");
  ...
  const normalizedCustomUrl = useMemo(() => normalizeBaseUrl(customUrl), [customUrl]);
  ...
  const checkServer = async () => {
    ...
    try {
      ...
      if (status.running) {
        addStatusMessage(`Server detected: ${status.server_name || serverType} v${status.version || "unknown"}`);
      }
    } catch (error) {
  ...
  const startServer = async () => {
    ...
    try {
      ...
      addStatusMessage(`Server ${result.status || "started"}${result.path ? `: ${result.path}` : ""}`);
  ...
  const importLibraries = async () => {
    ...
    try {
      ...
      addStatusMessage(`Imported ${result.sources_imported || 0} library sources from ${result.libraries_found || 0} libraries`);
    } catch (error) {
  ...
  const checkEmbyCompat = async () => {
    try {
      const result = await invoke<any>("check_emby_compat", { baseUrl: normalizedCustomUrl });
  ...
  return (
    <div className="space-y-5">
      <div className="glass-panel p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <select
              value={serverType}
              onChange={(event) => setServerStatus(serverRunning, event.target.value, normalizedCustomUrl)}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <div className="glass-panel-2 p-4 rounded-lg">
            ...
            {serverInfo && (
              <div className="space-y-1 text-xs text-cv-subtext">
                <div>Name: <span className="text-cv-text">{serverInfo.server_name || "Unknown"}</span></div>
                <div>Version: <span className="text-cv-text">{serverInfo.version || "Unknown"}</span></div>
              </div>
            )}
          ...
          <div className="glass-panel-2 p-4 rounded-lg">
            ...
            <input value={customUrl} onChange={(event) => setCustomUrl(event.target.value)} className="cv-input mb-2" />
            ...
            <div className="flex gap-2 mb-2">
              ...
              />
              <button onClick={() => openLink(normalizedCustomUrl)} className="cv-btn cv-btn-secondary text-xs shrink-0">
                <ExternalLink size={12} /> Key
              </button>
            </div>
            <div className="text-[10px] text-cv-subtext mb-3">
              {serverType === "emby" ? "MS-B: Dashboard > Advanced > Access Keys." : "MS-C: Dashboard > Advanced > Access Keys."}
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button onClick={importLibraries} className="cv-btn cv-btn-secondary text-xs">
                <Import size={12} /> Import
              </button>
              <button onClick={() => copyText(normalizedCustomUrl, "Server URL")} className="cv-btn cv-btn-secondary text-xs">
      ...
      <div className="glass-panel p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-bold flex items-center gap-2">
            <Globe size={16} className="text-cv-accent" /> Public Access Settings
          </h3>
          <button onClick={lookupPublicIp} className="cv-btn cv-btn-secondary text-xs py-1.5">
            <Wifi size={12} className={publicLookupBusy ? "animate-pulse" : ""} /> Lookup Public IP
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          <div>
            ...
            <input value={publicIp} onChange={(event) => setPublicIp(event.target.value)} className="cv-input" placeholder="Auto lookup or enter domain" />
          </div>
          <div>
            <label className="section-label">Public Port</label>
            <input value={publicPort} onChange={(event) => setPublicPort(event.target.value)} className="cv-input" placeholder="8096" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={publicAccessUrl || "Not configured"} readOnly className="cv-input" />
          </div>
        </div>

        <div className="flex flex-wrap gap-2 mt-4">
          <button onClick={applyPublicUrlAsServer} className="cv-btn cv-btn-primary text-xs">
            <Settings size={12} /> Use as Server URL
          </button>
          <button onClick={() => copyText(publicAccessUrl, "Public URL")} className="cv-btn cv-btn-secondary text-xs">
            <Clipboard size={12} /> Copy Public URL
          </button>
          <button onClick={() => publicAccessUrl && openLink(publicAccessUrl)} className="cv-btn cv-btn-secondary text-xs">
            <ExternalLink size={12} /> Open Public URL
          </button>
          <button onClick={() => copyText(`${publicPort}/tcp`, "Port rule")} className="cv-btn cv-btn-secondary text-xs">
            <Link2 size={12} /> Copy Port Rule
          </button>
        </div>

        <div className="mt-3 text-[11px] text-cv-subtext leading-relaxed">
          Public access usually requires router port forwarding, firewall allowance, and a stable IP or DNS hostname. Keep HTTPS enabled when your reverse proxy or certificate setup supports it.
        </div>
      </div>
