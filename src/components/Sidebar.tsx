...
function NavButton({ item, collapsed }: { item: NavItem; collapsed: boolean }): JSX.Element {
  ...
  return (
    <motion.button
      ...
      className={`group relative flex w-full items-center gap-3 overflow-hidden rounded-2xl px-3 py-3 text-left outline-none transition-colors focus-visible:ring-2 focus-visible:ring-cv-accent/70 ${isActive ? "text-white" : "text-cv-subtext hover:text-cv-text"}`}
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col">
        <div className="flex h-28 shrink-0 items-center gap-3 overflow-hidden border-b border-white/10 px-3">
          <div className="relative grid h-16 w-16 shrink-0 place-items-center rounded-[24px] border border-white/15 bg-white/[0.06] shadow-[0_0_34px_rgba(0,234,255,0.20)]">
            <img src="/branding/cinavault-logo.png" alt="CinaVault Premium" className="h-12 w-12 rounded-2xl object-cover" />
          </div>
          {!sidebarCollapsed && (
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.34em] text-cyan-200"><Film size={12} /> Media Center</div>
              <div className="truncate text-2xl font-black tracking-tight">CinaVault</div>
              <div className="mt-1 text-[10px] uppercase tracking-[0.18em] text-cv-subtext">Premium Online</div>
            </div>
          )}
        </div>
        ...
        <nav className="relative flex-1 overflow-y-auto px-2 py-3">
          <div className="mb-4">
            {!sidebarCollapsed && <div className="px-3 pb-1 text-[10px] font-bold uppercase tracking-[0.24em] text-cv-subtext/70">Casting</div>}
            <button
              type="button"
              onClick={() => window.dispatchEvent(new Event("cinavault:open-casting"))}
              className="group relative flex w-full items-center gap-3 rounded-2xl border border-cyan-200/15 bg-cyan-200/[0.07] px-3 py-3 text-left text-cyan-100 hover:bg-cyan-200/[0.12]"
              title={sidebarCollapsed ? "Casting" : undefined}
            >
              <span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-cyan-100/20 bg-black/30"><Cast size={20} /></span>
              {!sidebarCollapsed && (
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[14px] font-bold">Casting</span>
                  <span className="block text-[9px] uppercase tracking-[0.24em] opacity-65">Devices</span>
                </span>
              )}
            </button>
          </div>
          ...
          {groups.map((group) => (
            <div key={group} className="mb-4 last:mb-0">
              {!sidebarCollapsed && <div className="px-3 pb-1 text-[10px] font-bold uppercase tracking-[0.24em] text-cv-subtext/70">{group}</div>}
        ...
        <button
          ...
          className="relative z-10 m-2 flex h-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.04] text-cv-subtext hover:bg-white/[0.08] hover:text-cv-text"
