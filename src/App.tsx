...
// CinaVault Premium — Build 149 Media Center Application Shell
...
const TAB_TITLES: Record<TabId, { eyebrow: string; title: string; subtitle: string }> = {
  home: { eyebrow: "Media Center", title: "Home", subtitle: "Featured library shelves, recent additions, and playback entry points." },
  sources: { eyebrow: "Library", title: "Media Sources", subtitle: "Add, scan, repair, and verify source folders." },
  downloads: { eyebrow: "Queue", title: "Downloads", subtitle: "Track incoming media and library acquisition tasks." },
  livetv: { eyebrow: "Channels", title: "Live TV", subtitle: "Live streams, guide data, and viewing controls." },
  server: { eyebrow: "Core", title: "Server", subtitle: "CinaVault server status, services, and media delivery controls." },
  security: { eyebrow: "Guard", title: "Security", subtitle: "Protect access, credentials, and runtime configuration." },
  remote: { eyebrow: "Access", title: "Remote Access", subtitle: "External connection and relay configuration." },
  advanced: { eyebrow: "Tools", title: "Advanced", subtitle: "Power-user repair, diagnostics, and build controls." },
  cloud: { eyebrow: "Storage", title: "Cloud & NAS", subtitle: "Network storage, cloud library paths, and sync options." },
  plugins: { eyebrow: "Add-ons", title: "Plugins", subtitle: "Installed media helpers and permanent startup plugin coverage." },
  ai: { eyebrow: "Assistant", title: "AI Diagnostics", subtitle: "Inference, provider checks, metadata posting, and guided repairs." },
  settings: { eyebrow: "Config", title: "Settings", subtitle: "Theme, application, and persistent user preferences." },
};
  ...
  return (
    <div className="app-shell cv-app min-h-screen flex overflow-hidden bg-[#030813] text-cv-text">
      ...
      >
        ...
        <CastButton />
        <div data-testid="cinavault-permanent-media-plugins" style={{ display: "none" }}>{startupPluginsReady ? "Permanent media plugins ready" : "Permanent media plugins not ready"}</div>
        <div data-testid="cinavault-feature-suite" style={{ display: "none" }}>{featureCount} enabled media server features</div>
        <div data-testid="cinavault-proprietary-server" style={{ display: "none" }}>{serverName}</div>
        ...
        <main className="relative flex-1 flex flex-col overflow-hidden rounded-[32px] border border-white/10 bg-[#06101d]/82 shadow-[0_30px_100px_rgba(0,0,0,0.62)] backdrop-blur-xl">
          ...
          <section className="relative z-10 mx-4 mt-2 overflow-hidden rounded-[28px] border border-white/10 bg-[linear-gradient(105deg,rgba(0,0,0,0.48),rgba(255,255,255,0.08)),radial-gradient(circle_at_86%_22%,rgba(0,234,255,0.22),transparent_28%)] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]">
            ...
            <div className="relative z-10 grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-end">
              <div>
                <div className="text-[11px] font-bold uppercase tracking-[0.34em] text-cyan-200">{activeTitle.eyebrow}</div>
                <h1 className="mt-2 text-4xl font-black tracking-tight md:text-5xl">{activeTitle.title}</h1>
                <p className="mt-2 max-w-3xl text-sm text-cv-subtext md:text-base">{activeTitle.subtitle}</p>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="rounded-2xl border border-white/10 bg-black/25 p-3">
                  <div className="text-2xl font-black text-cv-text">{featureCount}</div>
                  <div className="text-[9px] uppercase tracking-[0.22em] text-cv-subtext">Features</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-black/25 p-3">
                  <div className="text-2xl font-black text-cv-text">{startupPluginsReady ? "On" : "Off"}</div>
                  <div className="text-[9px] uppercase tracking-[0.22em] text-cv-subtext">Plugins</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-black/25 p-3">
                  <div className="truncate text-lg font-black text-cv-text">{serverName}</div>
                  <div className="text-[9px] uppercase tracking-[0.22em] text-cv-subtext">Server</div>
                </div>
              </div>
            </div>
          </section>
