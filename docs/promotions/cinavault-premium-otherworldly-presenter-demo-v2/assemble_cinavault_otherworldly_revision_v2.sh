...
OUTDIR="$ROOT/cinavault-otherworldly-revision-v2-work"
OUT="$ROOT/cinavault-premium-otherworldly-presenter-demo-v2.mp4"
FONT=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf
mkdir -p "$OUTDIR"
rm -f "$OUTDIR"/seg*.mp4 "$OUTDIR/concat.txt" "$OUT"
make_seg(){
  local num="$1" src="$2" dur="$3" title="$4" subtitle="$5"
  ffmpeg -y -loglevel error -loop 1 -framerate 30 -t "$dur" -i "$src" \
    -vf "scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=0x040914,setsar=1,eq=saturation=1.08:contrast=1.04,zoompan=z='1+0.02*sin(on/80)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=1920x1080:fps=30,drawbox=x=0:y=0:w=1920:h=86:color=0x02040dcc:t=fill,drawtext=fontfile=$FONT:text='$title':fontcolor=0xD9FBFF:fontsize=30:x=72:y=25,drawtext=fontfile=$FONT:text='$subtitle':fontcolor=0x7DE8F4:fontsize=18:x=72:y=58" \
    -c:v libx264 -preset medium -crf 18 -pix_fmt yuv420p "$OUTDIR/seg${num}.mp4"
}
make_seg 01 "$ROOT/cinavault-humanoid-presenter-reference.png" 8 "CINAVAULT PREMIUM" "STEP INTO THE VAULT"
...
make_seg 04 "$ROOT/cinavault-humanoid-presenter-reference.png" 10 "A LIVING MEDIA UNIVERSE" "THE PRESENTER IS A CINEMATIC GUIDE"
...
make_seg 07 "$ROOT/cinavault-humanoid-presenter-reference.png" 8 "CINAVAULT PREMIUM" "THE VAULT IS WAITING"
...
sha256sum "$OUT" > "$ROOT/cinavault-premium-otherworldly-presenter-demo-v2-sha256.txt"
echo "$OUT"
