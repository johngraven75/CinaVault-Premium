...
test("media cards render uniform standard size with multiple cards per row", async ({ page }) => {
  const css = fs.readFileSync(path.resolve("src/styles/media-card-final-standard.css"), "utf8");
  ...
  await page.setContent(`<style>${css}</style><main class="media-grid">${cards}</main>`);
  ...
  const cardsInFirstRow = boxes.filter((box) => Math.round(box.y) === firstRowY);
