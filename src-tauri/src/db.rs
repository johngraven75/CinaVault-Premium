// CinaVault Premium — SQLite Database Layer (rusqlite)
...
pub struct MediaItem {
    ...
    pub media_type: String, // movie, tvshow, music, photo
    pub year: Option<i32>,
...
pub struct MediaSource {
    pub id: Option<i64>,
    pub path: String,
    pub source_type: String, // folder, drive, file
    pub name: String,
    pub enabled: bool,
...
impl Database {
    ...
    fn initialize_tables(&self) -> SqlResult<()> {
        ...
        let _ = self.conn.execute(
            "ALTER TABLE xtream_profiles ADD COLUMN output_format TEXT DEFAULT 'ts'",
            [],
        );
        let _ = self.conn.execute(
            "ALTER TABLE xtream_profiles ADD COLUMN user_agent TEXT DEFAULT ''",
            [],
        );
        ...
        let defaults = vec![
            ...
            ("cloud_onedrive_connected", "false"),
    ...
    pub fn get_media_items_data(&self, media_type: Option<&str>, limit: Option<i64>, offset: Option<i64>) -> SqlResult<Vec<MediaItem>> {
        ...
        let rows = if let Some(mt) = media_type {
            stmt.query_map(params![mt, lim, off], Self::row_to_media)?
        } else {
            stmt.query_map(params![lim, off], Self::row_to_media)?
        };
        rows.collect()
    }
