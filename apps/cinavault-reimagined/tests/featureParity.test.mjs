...
const requiredLegacyCapabilities = [
  ...
  ['Media scanner', 'src-tauri/src/scanner.rs', 'scan_library'],
...
test('legacy capability sources remain intact during migration', async () => {
  for (const [name, path, token] of requiredLegacyCapabilities) {
    const absolute = resolve(repoRoot, path);
    await access(absolute);
    const content = await readFile(absolute, 'utf8');
    assert.ok(content.includes(token), `${name} missing token ${token} in ${path}`);
  }
});
